...
void usb_midi_task()
{
    ...
    {
        if (buff[1] == (0x90 - (MIDI_CHANNEL - change_channel)))
        {
            if (buff[3] > 0)
            {
        ...
        if (buff[1] == (0x80 - (MIDI_CHANNEL - change_channel)))
        {
            note_off(buff[2]);
        }

        if (buff[1] == (0xE0 - (MIDI_CHANNEL - change_channel)))
        {
            midi_pitch_bend = buff[2] | (buff[3] << 7);
        }

        if (buff[1] == (0xB0 - (MIDI_CHANNEL - change_channel)))
        {
            if (buff[2] == 5)
            { // portamento time
            ...
            if (buff[2] == 3)
            { // midi channel change
                if (buff[3] >= 1 || buff[3] <= 16)
                {
                    for (int i = 0; i < NUM_VOICES; i++)
                    {
                        VOICE_GATE[i] = 0;
                    }
                    change_channel = buff[3];
...
void serial_midi_task()
{
    ...
    if (midi_serial_status == (0x90 - (MIDI_CHANNEL - change_channel)))
    {
        if (msb > 0)
        {
            note_on(lsb, msb);
    ...
    if (midi_serial_status == (0x80 - (MIDI_CHANNEL - change_channel)))
    {
        note_off(lsb);
    }

    if (midi_serial_status == (0xE0 - (MIDI_CHANNEL - change_channel)))
    {
        midi_pitch_bend = lsb | (msb << 7);
    }

    if (midi_serial_status == (0xB0 - (MIDI_CHANNEL - change_channel)))
    {
        if (lsb == 5)
        { // portamento time
            portamento_time = msb;
...
void note_on(uint8_t note, uint8_t velocity)
{
    if (STACK_VOICES < 2)
    {
        for (int i = 0; i < NUM_VOICES; i++)
        {
            if (VOICE_NOTES[i] == note && VOICE_GATE[i])
...
void led_blinking_task()
{
    if (board_millis() - LED_BLINK_START < 50)
        return;
